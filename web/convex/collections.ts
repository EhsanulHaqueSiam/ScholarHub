 ...
 * - Internal: recomputeAllCounts (daily cron for eventual consistency, D-90)
 */
...
/**
 * Recompute scholarship_count for all active collections (D-90).
 * Called by daily cron. Processes in batches of 10, schedules next batch if more.
 */
export const recomputeAllCounts = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const batch = allCollections.slice(startIndex, startIndex + batchSize);

    if (batch.length === 0) return { processed: 0, done: true };

    // Get published scholarships once for all collections in this batch
    const published = await ctx.db
      .query("scholarships")
      .withIndex("by_status", (q) => q.eq("status", "published"))
      .take(10000);
    ...
    // Schedule next batch if there are more collections
    const nextIndex = startIndex + batchSize;
    if (nextIndex < allCollections.length) {
      await ctx.scheduler.runAfter(0, internal.collections.recomputeAllCounts, {
        cursor: nextIndex,
      });
    }

    return { processed: batch.length, done: nextIndex >= allCollections.length };
  },
});
